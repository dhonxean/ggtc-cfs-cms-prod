<template lang="html">
	<div id="dashboard" v-if="loaded">
		...
		<div class="actions">
			<nuxt-link to="/resources/files/create" class="success button pointer">
				...
				<div class="wrapper">
					<div class="content">
						...
						<form class="content_middle form" @submit.prevent="fetchData()">
							...
							<div :class="[ 'group select bordered', (form.year_id.length > 0) ? 'filled' : '' ]">
								<label for="sample_v2">Year</label>
								<select class="input" name="year" v-model="form.year_id">
		...
		<table class="table">
			<thead>
				<tr>
					<th class="stick sort">
						<div :class="`label pointer ${sort.file_title ? 'asc' : 'desc'}`" @click="sort_table('file_title')">
							Title
						</div>
					</th>
					<th class="stick sort">
						<div :class="`label pointer ${sort.year ? 'asc' : 'desc'}`" @click="sort_table('year')">
							Year
						</div>
					</th>
					<th class="stick sort">
			...
			<tbody v-if="records.data.length > 0">
				<tr v-for="(data, key) in records.data" :key="key">
					<td>
						<nuxt-link :to="`/resources/files/${data.id}/update`" class="name pointer">
							{{ data.file_title }}
						</nuxt-link>
					</td>
					<td>{{ data.year }}</td>
					...
					<td class="buttons" width="210px">
						<div class="wrapper">
							<nuxt-link :to="`/resources/files/${data.id}/update`" class="item info pointer">
							...
							<div class="item ml cancel pointer" @click="toggleModalStatus({ type: 'delete_confirmation', status: true, item: { api: `v1/admin/resources/resource/delete/${data.id}`, item_type: 'file' } })">
...
<script>
	export default {
		data: () => ({
			...
			sort: {
				file_title: true, //true = asc | false = desc
				year: false,
				created_at: false,
			},
			has_search: true,
		...
		methods: {
			...
			fetchData() {
				...
				me.$axios.$post('v1/admin/resources/resource/list', me.form).then(({ res }) => {
					me.records = res
		...
		asyncData ({ $axios, store }) {
			...
			return $axios.$get('v1/admin/resources/year/list').then(({ res }) => {
				...
				return $axios.$post('v1/admin/resources/resource/list', form_data).then(({ res }) => {
					return {
						years: years,
						records: res
					}
