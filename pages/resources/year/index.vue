<template lang="html">
	<div id="dashboard" v-if="loaded">
		...
		<div class="actions">
			<nuxt-link to="/resources/year/create" class="success button pointer">
		...
		<table class="table">
			...
			<tbody v-if="records.length > 0">
				<tr v-for="(data, key) in records" :key="key">
					<td>
						<nuxt-link :to="`/resources/year/${data.id}/update`" class="name pointer">
							{{ data.year }}
						</nuxt-link>
					</td>
					<td :class="`bg ${data.featured ? 'success' : 'cancel' } upper`">
						<span>
					...
					<td class="buttons" width="210px">
						<div class="wrapper">
							<nuxt-link :to="`/resources/year/${data.id}/update`" class="item info pointer">
							...
							<div class="item ml cancel pointer" @click="toggleModalStatus({ type: 'delete_confirmation', status: true, item: { api: `v1/admin/resources/year/delete/${data.id}`, item_type: 'year' } })">
