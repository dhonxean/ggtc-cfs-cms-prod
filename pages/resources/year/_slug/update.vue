<template>
	<div id="dashboard" v-if="loaded">
		...
		<div class="actions">
			...
			<nuxt-link to="/resources/year/create" class="success ml ten button pointer">
		...
		<ValidationObserver tag="div" ref="form">
			<form id="form" class="form mb seventy" @submit.prevent="submit()">
				...
				<div class="buttons fixed">
					<nuxt-link to="/resrouces/year" class="cancel button half_width btn lg">Cancel</nuxt-link>
...
<script>
	export default {
			...
			submit () {
				...
				me.$refs.form.validate().then(success => {
					if (!success) {
						...
						me.$axios.$post(`v1/admin/resources/year/update/${me.$route.params.slug}`, form_data).then(({ res }) => {
		...
		asyncData ({ $axios, store, error, params }) {
			...
			return $axios.$get(`v1/admin/resources/year/info/${params.slug}`).then(({ res }) => {
				let year = res,
					web_image = [],
					mobile_image = []
				
				year.images.forEach((item, index) => {
