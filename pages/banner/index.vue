				...
				<tr v-for="(data, key) in records.data" :key="key">
					...
					<td class="buttons" width="210px">
						<div class="wrapper">
							...
							<div class="item ml cancel pointer" @click="toggleModalStatus({ type: 'delete_confirmation', status: true, item: { api: `v1/admin/banner/delete/${data.id}`, item_type: 'banner' } })">
...
<script>
	export default {
		...
		asyncData ({ $axios, store }) {
			...
			return $axios.$get('v1/admin/banner/list', form).then(({ res }) => {
				return {
					records: res
				}
