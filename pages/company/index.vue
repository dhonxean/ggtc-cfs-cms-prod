<template lang="html">
	<div id="dashboard" v-if="loaded">
		...
		<div class="actions">
			<nuxt-link to="/company/create" class="success button pointer">
		...
		<table class="table">
			...
			<tbody v-if="records.data.length > 0">
				<tr v-for="(data, key) in records.data" :key="key">
					<td>
						<nuxt-link :to="`/company/${data.id}/update`" class="name pointer">
							{{ data.name }}
						</nuxt-link>
					</td>
					<td class="buttons" width="210px">
						<div class="wrapper">
							<nuxt-link :to="`/company/${data.id}/update`" class="item info pointer">
							...
							<div class="item ml cancel pointer" @click="toggleModalStatus({ type: 'delete_confirmation', status: true, item: { api: `v1/admin/company/delete/${data.id}`, item_type: 'company' } })">
	...
	export default {
		...
		methods: {
			...
			fetchData() {
				...
				me.$axios.$post('v1/admin/company/get-all-company', me.form).then(({ res }) => {
					me.records = res
		...
		asyncData ({ $axios, store }) {
			...
			return $axios.$post('v1/admin/company/get-all-company', form).then(({ res }) => {
				return {
					records: res
				}
			}).catch(({ response: { data: { errors } } }) => {
