				...
				<tr v-for="(data, key) in records.data" :key="key">
					...
					<td class="buttons" width="210px">
						<div class="wrapper">
							...
							<div class="item ml cancel pointer" @click="toggleModalStatus({ type: 'delete_confirmation', status: true, item: { api: `admin/company/delete/${data.id}`, item_type: 'company' } })">
	...
	export default {
		...
		methods: {
			...
			fetchData() {
				...
				me.$axios.$post('admin/company/get-all-company', me.form).then(({ res }) => {
					me.records = res
		...
		asyncData ({ $axios, store }) {
			...
			return $axios.$post('admin/company/get-all-company', form).then(({ res }) => {
				return {
					records: res
				}
			}).catch(({ response: { data: { errors } } }) => {
