				...
				<tr v-for="(data, key) in records.data" :key="key">
					...
					<td class="buttons" width="210px">
						<div class="wrapper">
							...
							<div class="item ml cancel pointer" @click="toggleModalStatus({ type: 'delete_confirmation', status: true, item: { api: `v1/admin/currency-rate/delete/${data.id}`, item_type: 'currency' } })">
	...
	export default {
		...
		methods: {
			...
			fetchData() {
				...
				me.$axios.$post('v1/admin/currency-rate/get-all-currency', me.form).then(({ res }) => {
					me.records = res
		...
		asyncData ({ $axios, store }) {
			...
			return $axios.$post('v1/admin/currency-rate/get-all-currency', form_data).then(({ res }) => {
				return {
					records: res
				}
			}).catch(({ response: { data: { errors } } }) => {
